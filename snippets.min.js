...
all_tags=all_tags.concat(d);b=createSnippetHTML(b.title,b.id,b.content,b.meta,b.tags);$(".timeline").append(b)});new ClipboardJS(".copy-btn");renderTags()})}function getSheetId(){var a=document.getElementsByTagName("script");if(a=a[a.length-1].getAttribute("data-id"))return a;console.log("No sheet ID present!")}
...
e.meta,e.tags);$(".snippet-counter").html(c+" snippets found");$(".timeline").append(e)}})}))}
function renderTags(){$(".page-tags").empty();$(".page-tags").append("<button class='snippet-tag-link' onclick='filterOnTag(\"all\")'>all</button>");all_tags=all_tags.filter(function(a,b,c){return c.indexOf(a)===b});all_tags.sort();$.each(all_tags,function(a,b){10>a&&$(".page-tags").append("<button class='snippet-tag-link' onclick='filterOnTag(\""+b+"\")'>"+b+"</button>");10==a&&$(".page-tags").append("<button class='snippet-tag-link' onclick='renderAllTags()'>more ...</button>")})}
function renderAllTags(){$(".page-tags").empty();$(".page-tags").append("<button class='snippet-tag-link' onclick='filterOnTag(\"all\")'>all</button>");$.each(all_tags,function(a,b){$(".page-tags").append("<button class='snippet-tag-link' onclick='filterOnTag(\""+b+"\")'>"+b+"</button>")})}function getUrlParam(a,b){b||(b=location.href);a=a.replace(/[\[]/,"\\[").replace(/[\]]/,"\\]");var c=(new RegExp("[\\?&]"+a+"=([^&#]*)")).exec(b);return null==c?null:c[1]}
function createSnippet(a,b,c,f,d){return{title:a,id:b,content:c,meta:f,tags:d}}
function createSnippetHTML(a,b,c,f,d){try{var e=d.split(",").map(function(a){return a.trim()})}catch(k){e=d}var g="",h="";$.each(e,function(a,b){g+='<span class="snippet-tags float-right">'+b+"</span>";h+=b+" "});return'<li id="'+b+'" class="snippet-li '+h+'">\n        <span class="snippet-title">'+a+"</span>\n         "+g+'\n        <div class="snippet-content">'+c+'</div>\n        <div class="snippet-footer">'+f+'\n        <button class="float-right copy-btn"  data-clipboard-action="copy" data-clipboard-target="#'+b+
